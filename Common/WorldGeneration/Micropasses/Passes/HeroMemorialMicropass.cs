...
internal class HeroMemorialMicropass : Micropass
{
	...
	{
		progress.Message = Language.GetTextValue("Mods.SpiritReforged.Generation.HerosMemorial");
	...
	retry:
		...
		if (!TileObject.CanPlace(x, y - 2, ModContent.TileType<HerosMemorialStatue>(), 0, 0, out var _, true))
			goto retry;
		...
		WorldGen.PlaceObject(x, y - 2, ModContent.TileType<HerosMemorialStatue>(), true, direction: WorldGen.genRand.NextBool() ? -1 : 1);
