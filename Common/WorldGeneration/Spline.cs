...
/// Source: https://github.com/SCToolsfactory/SCJMapper-V2/blob/master/OGL/CubicSpline.cs
/// </summary>
public static class Spline
{
	public static Vector2[] CreateSpline(Vector2[] importantPoints, int count)
	{
		double[] xs = new double[importantPoints.Length];
		double[] ys = new double[importantPoints.Length];

		for (int i = 0; i < importantPoints.Length; i++)
		{
			xs[i] = importantPoints[i].X;
			ys[i] = importantPoints[i].Y;
		}

		(double[] newXs, double[] newYs) = InterpolateXY(xs, ys, count);
