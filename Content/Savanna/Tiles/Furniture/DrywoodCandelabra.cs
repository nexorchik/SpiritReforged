...
public class DrywoodCandelabra : ModTile, IAutoloadTileItem
{
	...
	public override void HitWire(int i, int j)
	{
		...
		(i, j) = (i - Framing.GetTileSafely(i, j).TileFrameY / 18, j - Framing.GetTileSafely(i, j).TileFrameX % fullWidth / 18);
		...
		for (int y = 0; y < 2; y++)
		{
			for (int x = 0; x < 2; x++)
			{
				...
				tile.TileFrameX += (short)((tile.TileFrameX < fullWidth) ? fullWidth : -fullWidth);
	...
	public override void PostDraw(int i, int j, SpriteBatch spriteBatch)
	{
		...
		var source = new Rectangle(tile.TileFrameX, tile.TileFrameY, 16, tile.TileFrameY > 0 ? 18 : 16);
