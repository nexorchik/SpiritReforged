...
public class DuelistLegacy : ModItem
{
	...
	public override void SetStaticDefaults() => ItemID.Sets.ShimmerTransformToItem[Type] = ModContent.ItemType<LadyLuck.LadyLuck>();
	...
	public override bool Shoot(Player player, EntitySource_ItemUse_WithAmmo source, Vector2 position, Vector2 velocity, int type, int damage, float knockback) 
	{
		if (player.altFunctionUse == 2)
		{
			...
			if (ChargeReady)
			{
				SoundEngine.PlaySound(new SoundStyle("SpiritReforged/Assets/SFX/Shotgun") with { PitchVariance = 0.2f, Volume = 0.4f}, player.Center);

				for (int i = 0; i < 15; i++)
				{
					var dust = Dust.NewDustDirect(player.Center + direction * 20, 0, 0, ModContent.DustType<DuelistBubble2>());
					dust.velocity = direction.RotatedBy(Main.rand.NextFloat(-0.4f, 0.4f)) * Main.rand.NextFloat(2, 10);
				}

                player.SimpleShakeScreen(2f, 1f, 6, 800, "DuelistLegacy");
			...
			else
			{
				...
				SoundEngine.PlaySound(new SoundStyle("SpiritReforged/Assets/SFX/ShotgunLoud") with { PitchVariance = 0.6f, Volume = 0.46f }, player.Center);

				for (int i = 0; i < 10; i++)
				{
					var dust = Dust.NewDustDirect(player.Center + direction * 20, 0, 0, ModContent.DustType<DuelistSmoke>());
		...
		else
		{
			...
			var proj = Projectile.NewProjectileDirect(source, position + direction * 20 + direction.RotatedBy(-1.57f * player.direction) * 20, Vector2.Zero, ModContent.ProjectileType<DuelistSlash>(), damage, knockback, player.whoAmI);
...
internal class DuelistSlash : ModProjectile
{
    ...
    public int MaxFrames => Phase switch
    {
        0 => 11,
        1 => 11,
        2 => 8,
    ...
    public int SwingDirection => Phase switch
    {
	...
	public override bool? Colliding(Rectangle projHitbox, Rectangle target)
	{
		Vector2 dir = rotation.ToRotationVector2();
		float collisionPoint = 0;
		return Collision.CheckAABBvLineCollision(target.TopLeft(), target.Size(), Player.Center, Player.Center + dir * Projectile.width, Projectile.height, ref collisionPoint);
    }
	...
	public override void AI()
	{
		...
		if (!initialized)
		{
			...
			if (Empowered)
			{
				...
				SoundEngine.PlaySound(new SoundStyle("SpiritReforged/Assets/SFX/RoughSlash") with { PitchVariance = 0.1f, Volume = 0.4f }, Projectile.Center);
                Player.SimpleShakeScreen(2f, 1f, 6, 800, "DuelistLegacy");
            }
		}
		...
		if (!Empowered)
		{
			if (Projectile.frame is > 2 and < 5)
			{
		...
		if (Projectile.frame is >= 2 and < 7)
			Projectile.friendly = true;
		else
	...
	public override bool PreDraw(ref Color lightColor)
	{
		...
		{
			0 => ModContent.Request<Texture2D>(Texture + "One", AssetRequestMode.ImmediateLoad).Value,
			1 => ModContent.Request<Texture2D>(Texture + "Two", AssetRequestMode.ImmediateLoad).Value,
			2 => ModContent.Request<Texture2D>(Texture + "Special", AssetRequestMode.ImmediateLoad).Value,
...
internal class DuelistGun : ModProjectile
{
	...
	public override LocalizedText DisplayName => Language.GetText("Mods.SpiritReforged.Items.DuelistLegacy.DisplayName");
	...
	public override bool PreDraw(ref Color lightColor)
	{
		...
		if (player.direction == 1)
		{
			...
			Main.spriteBatch.Draw(texture, position, new Rectangle(0, y2, texture.Width, height), lightColor * .91f, CurrentDirection.ToRotation(), new Vector2(texture.Width / 2f, height / 2f), Projectile.scale, effects1, 0.0f);
			Main.spriteBatch.Draw(texture, position, new Rectangle(0, y2, texture.Width, height), Color.Gray * .05f, CurrentDirection.ToRotation(), new Vector2(texture.Width / 2f, height / 2f), Projectile.scale * 1.2f, effects1, 0.0f);
		}
		else
		{
			SpriteEffects effects1 = SpriteEffects.FlipHorizontally;
			Main.spriteBatch.Draw(texture, position, new Rectangle(0, y2, texture.Width, height), lightColor * .91f, CurrentDirection.ToRotation() - 3.14f, new Vector2(texture.Width / 2f, height / 2f), Projectile.scale, effects1, 0.0f);
			Main.spriteBatch.Draw(texture, position, new Rectangle(0, y2, texture.Width, height), Color.Gray * .05f, CurrentDirection.ToRotation() - 3.14f, new Vector2(texture.Width / 2f, height / 2f), Projectile.scale * 1.2f, effects1, 0.0f);
...
internal class DuelistBlast : ModProjectile
{
	...
	public override bool? Colliding(Rectangle projHitbox, Rectangle target)
	{
		...
		float collisionPoint = 0;
		return Collision.CheckAABBvLineCollision(target.TopLeft(), target.Size(), Projectile.Center, Projectile.Center + lineDirection * Projectile.width, Projectile.height, ref collisionPoint);
    }
	...
	public override void ModifyHitNPC(NPC target, ref NPC.HitModifiers modifiers) => modifiers.HitDirectionOverride = direction;
}
