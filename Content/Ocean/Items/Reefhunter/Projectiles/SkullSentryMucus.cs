...
public class SkullSentryMucus : ModProjectile
{
	...
	public override bool PreDraw(ref Color lightColor)
	{
		...
		for (int i = trailLength - 1; i >= 0; i--)
		{
			...
			float opacity = EaseFunction.EaseCubicOut.Ease(progress);
	...
	public void MakeDust(int dustCount, float speed, float scale, int alpha, float radius)
	{
		for (int i = 0; i < dustCount; ++i)
		{
			...
			var d = Dust.NewDustPerfect(pos, DustID.Blood, velocity, alpha, new Color(250, 173, 173), Main.rand.NextFloat(0.66f, 1f) * scale);
			d.noGravity = true;
		}
	}
}
